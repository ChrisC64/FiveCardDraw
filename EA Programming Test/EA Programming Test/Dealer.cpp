...
Dealer::Dealer() : m_bIsDealing(true), 
m_iHandSize(5),
m_iNumOfPlayers(2),
...
{
    m_Deck = new Deck();
    m_Cards.resize(m_iHandSize);
    m_DealtCards = &m_Cards;
...
void Dealer::SetCategory(Player *p, Hand::CATEGORY eCategory)
{
...
// ADD PLAYERS //
void Dealer::AddPlayers(Player* p)
{
    // If empty, resize and add players
    if (m_vPlayers.empty())
    {
        m_vPlayers.resize(1, p);
        m_iNumOfPlayers = 1;
        return;
    }
...
void Dealer::PrintWinner(Player *p)
{
    std::cout << "Congratulations! [" << p->GetName();
    std::cout << "] You won!" << std::endl;
}

void Dealer::PrintTie(Player *p)
{
    std::cout << p->GetName();
    std::cout << " ";
}
...
void Dealer::FindHighValue(std::list<Player*> *playerList)
{
    if (playerList->empty())
    {
        printf_s("Cannot findValue(): HigcardList is empty!");
...
void Dealer::Compare(std::list<Player*> *playerList)
{
    std::vector<Card> *currHand;
    std::vector<Card> *secondHand;
    bool winnerFound = false;
    int highNum = 0;
    std::list<Player*> winningPlayer;
    for (auto p = playerList->begin(); p != playerList->end(); ++p)
    {
